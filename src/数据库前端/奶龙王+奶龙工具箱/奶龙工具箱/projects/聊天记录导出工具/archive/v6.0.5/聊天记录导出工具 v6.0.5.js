...
(function() {
    ...
    $('head').append(`
        <style id="${STYLE_ID}">
            ...
            .cet-tab-content#tab-txt { max-height: 550px; min-height: 0; display: none; flex-direction: column; overflow: hidden; flex: 1; }
            ...
            @media (max-width: 768px), (pointer: coarse) {
                ...
                .cet-wrapper { padding: 6px; gap: 5px; height: 100%; min-height: 0; }
                .cet-card {
                    height: 100%;
                    min-height: 0;
                }
    ...
    async function showExportUI() {
        try {
            ...
            const setupMobileScrollGuards = ($root) => {
                const scrollSelector = '.cet-tab-content, .cet-txt-scroll, .cet-search-results, .cet-tags-list, .cet-scan-container, .cet-textarea';
                const rootEl = $root[0];
                let activeScroller = null;

                const findScrollable = (target) => {
                    let el = target.closest ? target.closest(scrollSelector) : null;
                    while (el && rootEl.contains(el)) {
                        if (el.scrollHeight > el.clientHeight + 1) return el;
                        el = el.parentElement ? el.parentElement.closest(scrollSelector) : null;
                    }
                    return null;
                };
                ...
                const onTouchStart = (e) => {
                    ...
                    activeScroller = findScrollable(e.target);
                };

                const onTouchMove = (e) => {
                    if (!activeScroller || !e.touches || e.touches.length !== 1) return;
                    e.stopPropagation();
                };
                ...
                const onWheel = (e) => {
                    ...
                    const canMove = (deltaY < 0 && !atTop) || (deltaY > 0 && !atBottom);
                    if (!canMove) return;
                ...
                rootEl.addEventListener('touchstart', onTouchStart, { passive: true, capture: true });
                rootEl.addEventListener('touchmove', onTouchMove, { passive: true, capture: true });
                rootEl.addEventListener('touchend', onTouchEnd, { passive: true, capture: true });
                rootEl.addEventListener('touchcancel', onTouchEnd, { passive: true, capture: true });
                rootEl.addEventListener('wheel', onWheel, { passive: false, capture: true });
            };

            setupMobileScrollGuards($wrapper);
