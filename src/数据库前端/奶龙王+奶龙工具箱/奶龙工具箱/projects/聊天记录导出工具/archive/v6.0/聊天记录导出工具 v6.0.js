...
(function() {
    ...
    $('head').append(`
        <style id="${STYLE_ID}">
            ...
            .cet-card { display: flex; flex-direction: column; height: 100%; overflow: hidden; }
            ...
            /* 主内容容器 - 包含滚动区和底部按钮 */
            .cet-main-body { display: flex; flex-direction: column; flex: 1; overflow: hidden; gap: 8px; }

            /* 文本导出tab的特殊容器 - 固定550px */
            .cet-tab-content#tab-txt { max-height: 550px; display: none; flex-direction: column; overflow: hidden; }
            .cet-tab-content#tab-txt.active { display: flex; }
            .cet-tab-content#tab-txt .cet-txt-scroll { flex: 1; overflow-y: scroll; overflow-x: hidden; padding-right: 5px; }

            /* 其他tab正常显示 */
            .cet-tab-content#tab-nav, .cet-tab-content#tab-json { display: none; flex-direction: column; gap: 8px; flex: 1; overflow-y: auto; overflow-x: hidden; padding-right: 5px; }
    ...
    const Logic = {
        ...
        scanUsedTags: () => {
            ...
            msgs.forEach(msg => {
                ...
                const matches = [...msg.message.matchAll(/<([\u4e00-\u9fa5a-zA-Z0-9_\-\.]+)(?:\s[^>]*)?>([\s\S]*?)<\/\1>/g)];
                matches.forEach(m => {
        ...
        try {
                ...
                try {
                    ...
                    if (!isFullExport) {
                        ...
                        raw.forEach(t => {
                             const rx = Logic.parseRegex(t);
                             if (rx) {
                                 extractRegexList.push(rx);
                             } else {
                                 if (t.includes(':') && !t.startsWith('<')) {
                                     const colonIdx = t.indexOf(':');
                                     const tName = t.substring(0, colonIdx).trim();
                                     const kWord = t.substring(colonIdx + 1).trim();
                                     if (!extractRules[tName]) extractRules[tName] = [];
                                     if (extractRules[tName] !== null) extractRules[tName].push(kWord);
                                 } else {
                                     extractRules[t] = null;
                                 }
                             }
                        });
                        extractTagsList = Object.keys(extractRules);
                    }
                    let excludeTagsList = [];
                    if (excludeInput) {
                        excludeTagsList = excludeInput.split(/[,，]/).map(t => t.trim()).filter(t => t && !t.startsWith('</') && !t.includes(':'));
                    }
                    ...
                    freshMsgs.forEach((msg, idx) => {
                        ...
                        // ★★★ 标签闭合检测（用原始消息检测）★★★
                        if (opts.checkMissing && allCheckTags.length > 0 && msg.message) {
                            for (let tag of allCheckTags) {
                                const escapedTag = Logic.escapeRegex(tag);

                                const openRegex = new RegExp(`<${escapedTag}(?:\\s[^>]*)?>`, 'gi');
                                const closeRegex = new RegExp(`<\\/${escapedTag}\\s*>`, 'gi');

                                const openMatches = msg.message.match(openRegex) || [];
                                const closeMatches = msg.message.match(closeRegex) || [];

                                if (openMatches.length !== closeMatches.length) {
                                    allErrors.push(`#${msgId ?? '未知'} 标签 <${tag}> 结构异常 (开始: ${openMatches.length}, 结束: ${closeMatches.length})`);
                                }

                                const brokenOpenRegex = new RegExp(`<${escapedTag}(?:\\s[^>]*)?$`, 'mi');
                                if (brokenOpenRegex.test(msg.message)) {
                                    allErrors.push(`#${msgId ?? '未知'} 标签 <${tag}> 结构异常 (截断: 标签未闭合)`);
                                }
                            }
                        }
            ...
            $wrapper.find('#btn-export-epub').on('click', async function() {
                try {
                    ...
                    let extractRules = {}, extractTagsList = [], extractRegexList = [];
                    if (!isFullExport) {
                        const raw = tagInput.split(/[,，]/).map(t => t.trim()).filter(t => t);
                        raw.forEach(t => {
                             const rx = Logic.parseRegex(t);
                             if (rx) extractRegexList.push(rx);
                             else {
                                 if (t.includes(':') && !t.startsWith('<')) {
                                     const colonIdx = t.indexOf(':');
                                     const tName = t.substring(0, colonIdx).trim(), kWord = t.substring(colonIdx + 1).trim();
                                     if (!extractRules[tName]) extractRules[tName] = [];
                                     if (extractRules[tName] !== null) extractRules[tName].push(kWord);
                                 } else extractRules[t] = null;
                             }
                        });
                        extractTagsList = Object.keys(extractRules);
                    }
                    let excludeTagsList = excludeInput ? excludeInput.split(/[,，]/).map(t => t.trim()).filter(t => t && !t.startsWith('</') && !t.includes(':')) : [];
                    ...
                    freshMsgs.forEach((msg, idx) => {
                        ...
                        if (opts.checkMissing && allCheckTags.length > 0 && msg.message) {
                            for (let tag of allCheckTags) {
                                const escapedTag = Logic.escapeRegex(tag);
                                const openMatches = msg.message.match(new RegExp(`<${escapedTag}(?:\\s[^>]*)?>`, 'gi')) || [];
                                const closeMatches = msg.message.match(new RegExp(`<\\/${escapedTag}\\s*>`, 'gi')) || [];
                                if (openMatches.length !== closeMatches.length) allErrors.push(`#${msgId} 标签 <${tag}> 结构异常`);
                                if (new RegExp(`<${escapedTag}(?:\\s[^>]*)?$`, 'mi').test(msg.message)) allErrors.push(`#${msgId} 标签 <${tag}> 被截断`);
                            }
                        }
