...
(function() {
    ...
    $('head').append(`
        <style id="${STYLE_ID}">
            ...
            .cet-tab-content#tab-txt, .cet-tab-content#tab-nav { max-height: 550px; display: none; flex-direction: column; overflow: hidden; }
            ...
            .cet-tab-content#tab-nav.active { display: flex; }
            .cet-tab-content#tab-nav .cet-nav-scroll { flex: 1; overflow-y: scroll; overflow-x: hidden; padding-right: 5px; }

            /* 其他tab正常显示 */
            .cet-tab-content#tab-json { display: none; flex-direction: column; gap: 8px; flex: 1; overflow-y: auto; overflow-x: hidden; padding-right: 5px; }
            .cet-tab-content#tab-json.active { display: flex; }

            /* 滚动条样式 - 始终占位 */
            .cet-tab-content::-webkit-scrollbar, .cet-txt-scroll::-webkit-scrollbar, .cet-nav-scroll::-webkit-scrollbar { width: 5px; }
            .cet-tab-content::-webkit-scrollbar-track, .cet-txt-scroll::-webkit-scrollbar-track, .cet-nav-scroll::-webkit-scrollbar-track { background: transparent; }
            .cet-tab-content::-webkit-scrollbar-thumb, .cet-txt-scroll::-webkit-scrollbar-thumb, .cet-nav-scroll::-webkit-scrollbar-thumb { background: rgba(0,0,0,0.2); border-radius: 2px; }
            ...
            .${BTN_CLASS} { cursor: pointer; opacity: 0.4; margin-left: 5px; display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; min-width: 24px; min-height: 24px; transition: all 0.2s; touch-action: manipulation; -webkit-tap-highlight-color: transparent; }
            .${BTN_CLASS} i { pointer-events: none; }
    ...
    const Logic = {
        ...
        injectBookmarkButtonToMessage: (mesElement) => {
            ...
            const handleBookmarkTap = (e) => {
                e.preventDefault();
                e.stopPropagation();
                const now = Date.now();
                if (e.type === 'click' && now - touchHandledAt < 700) return;
                if (e.type === 'touchend' || e.type === 'pointerup') touchHandledAt = now;
                Logic.toggleBookmark(mesId);
            };
            $btn.on('click touchend pointerup', handleBookmarkTap);
