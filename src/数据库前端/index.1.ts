    ...
    mapItems.forEach(item => {
      if (item.type === 'element') {
        ...
        elG.addEventListener('click', e => {
          ...
          if (!state.isEditing) {
            e.stopPropagation();
            const rect = elG.getBoundingClientRect(); // 使用elG元素本身的坐标
...
async function injectMapTables(): Promise<boolean> {
  ...
  try {
    ...
    if (!existing.hasLocation) {
      ...
      const locTable = {
        ...MAP_TEMPLATES.locationTable,
        uid: locUid,
        name: '主要地点表', // 不带标记，保持兼容性
      };
      fullData[locUid] = locTable;
      dbg('[地图注入] 已添加主要地点表:', locUid);
    }

    // 如果没有地图元素表，注入它
    if (!existing.hasElement) {
      const elUid = generateTableUid();
      const elTable = {
        ...MAP_TEMPLATES.elementTable,
        uid: elUid,
        name: '地图元素表', // 不带标记，保持兼容性
      };
      fullData[elUid] = elTable;
      dbg('[地图注入] 已添加地图元素表:', elUid);
    }
...
/**
 * 从数据库移除浮岛注入的地图表格
 * 注意：只移除带有浮岛标记的表格，不影响用户自己创建的地图表格
 */
async function removeMapTables(): Promise<boolean> {
  ...
  try {
    ...
    Object.keys(fullData).forEach(key => {
      ...
      if (table && table.name && table.name.includes(MAP_TABLE_MARKER)) {
        keysToRemove.push(key);
        dbg('[地图移除] 标记移除:', table.name);
      }
    });
    ...
    if (removed) {
      ...
      showToast('未找到可移除的地图表格');
      dbg('[地图移除] 未找到浮岛标记的表格');
    }

    return removed;
  } catch (e) {
    console.error('[地图移除] 失败:', e);
