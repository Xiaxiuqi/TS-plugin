...
export const MAP_TEMPLATES = {
  ...
  locationTable: {
    ...
    "sourceData": {
      ...
      "deleteNode": "【绝对禁止删除】除非该地点在剧情中被彻底毁灭（如爆炸、沉没），否则严禁删除任何地点！角色离开、前往新区域或进入子区域时，必须保留原地点数据以便返回。切记：不要因为角色不在该地点就删除它！",
      "updateNode": "地点的环境描述等信息发生变化时更新。如果不确定地点名称，请不要随意修改地点名称，以免导致关联的墙壁和设施数据丢失。",
  ...
  elementTable: {
    ...
    "sourceData": {
      ...
      "deleteNode": "【禁止误删】仅在实体被彻底消灭、摧毁或永久移除时删除。角色离开或物品暂时取走时，不要删除，应更新其状态为'不在场'或类似描述。",
  ...
  wallsTable: {
    ...
    "sourceData": {
      ...
      "initNode": "当创建或发现新的主要地点时，必须同步创建该地点的墙壁结构。**重要：必须确保【场景ID】与【主要地点表】中的【地点名称】完全一致。**",
      "deleteNode": "【绝对禁止删除】只要对应的主要地点存在，就严禁删除其墙壁数据！严禁因为角色离开或不在该区域就删除墙壁数据！",
      "updateNode": "仅在场景内结构发生重大变化（如墙壁倒塌、扩建）时更新。**严禁修改【场景ID】，除非【主要地点表】中的对应地点名称也发生了变更。**",
  ...
  facilityTable: {
    ...
    "sourceData": {
      ...
      "initNode": "当角色进入一个新的微观场景时，如果该场景尚未生成布局，则创建新条目。**重要：必须确保【场景ID】与【主要地点表】中的【地点名称】完全一致。**",
      "deleteNode": "【谨慎删除】除非设施被彻底摧毁，否则尽量保留。如果上下文窗口紧张，可以仅删除长时间未访问的场景设施。",
      "updateNode": "场景内设施发生移动或改变时更新。**严禁修改【场景ID】，除非【主要地点表】中的对应地点名称也发生了变更。**",
      "insertNode": "发现新场景时添加。**注意：场景ID必须对应主要地点表中的地点名称。**"
    },
    "content": [[null, "场景ID", "名称", "类型", "X坐标", "Y坐标", "宽度", "高度", "旋转", "互动选项1", "互动选项2", "互动选项3"]],
    "exportConfig": {
      "enabled": false,
      "splitByRow": false,
      "entryName": "地图设施表",
