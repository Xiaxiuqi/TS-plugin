...
export function renderMacroMap(
  ...
  if (locsWithWalls.length > 0) {
      ...
      const overlayBounds = L.latLngBounds(
          L.latLng(-maxY - 100, minX - 100),
          L.latLng(-minY + 100, maxX + 100)
      );
      ...
      adjustedLocs.forEach((loc: MapLocation) => {
          ...
          if (locWalls.length > 0) {
              ...
              // 分离墙壁和门/开口
              const solidWalls = locWalls.filter(w => !w.type || (w.type.toLowerCase() !== 'door' && w.type.toLowerCase() !== 'gap' && w.type !== '门' && w.type !== '开口' && w.type !== '门洞'));
              const connections = locWalls.filter(w => w.type && (w.type.toLowerCase() === 'door' || w.type.toLowerCase() === 'gap' || w.type === '门' || w.type === '开口' || w.type === '门洞'));
              ...
              // 依然使用 Leaflet Rectangle，因为它处理交互很方便
               const rectBounds = L.latLngBounds(
                  L.latLng(-(loc.y + loc.height), loc.x),
                  L.latLng(-loc.y, loc.x + loc.width)
                );
...
export function renderMicroMap(
  ...
  if (walls && walls.length > 0) {
    ...
    try {
      const overlay = (L as any).svgOverlay(svgElement, svgBounds, {
          interactive: false
      }).addTo(map);
  ...
  if (facilities && facilities.length > 0) {
    ...
    facilities.forEach(item => {
      ...
      const polygon = (L as any).polygon(rotatedCorners, {
        color: '#666',
        weight: 1,
        fillColor: color,
        fillOpacity: 0.9,
