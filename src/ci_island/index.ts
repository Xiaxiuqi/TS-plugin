        ...
        newData.bodyStatus.forEach((item: any) => {
          if (updateCellOrComposite(row, headers, item.label, item.value)) {
            ...
            const colIdx = headers.findIndex(
              (hh: any) => hh && (String(hh) === item.label || String(hh).includes(item.label)),
            );
            if (colIdx !== -1) {
...
async function saveEventData(originalTask: any, newData: any) {
  ...
  const updates =
    Object.keys(rowData).length > 0 ? [{ tableName: targetTable.name, rowIndex: targetRowIndex, data: rowData }] : [];
...
async function saveSkillData(originalSkill: any, newData: any) {
  ...
  const updates =
    Object.keys(rowData).length > 0 ? [{ tableName: src.table, rowIndex: src.rowIdx, data: rowData }] : [];
