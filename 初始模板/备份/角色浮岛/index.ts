...
const STORAGE_CONTAINERS_KEY = 'ci_containers_v1'; // 容器内容物存储
...
const state = {
  ...
  // 容器管理相关
  containers: JSON.parse(safeGetItem(STORAGE_CONTAINERS_KEY, '{}')), // {容器名称: [元素名称列表]}
  isContainerMode: false, // 是否在容器整理模式
  selectedContainer: null as string | null, // 当前选中的容器
  elementsSidebar: false, // 元素侧边栏是否打开
};
...
// 判断元素是否为容器类型
function isContainerElement(element: any): boolean {
  const containerKeywords = [
    '容器', '储物箱', '箱子', '柜子', '背包', '仓库', 
    '武器架', '书架', '衣柜', '保险箱', '宝箱',
    'Container', 'Chest', 'Storage', 'Box', 'Rack'
  ];
  return containerKeywords.some(k => String(element.type || '').includes(k));
}

// 获取容器内的元素列表
function getContainerItems(containerName: string): string[] {
  return state.containers[containerName] || [];
}

// 添加元素到容器
function addItemToContainer(containerName: string, itemName: string) {
  if (!state.containers[containerName]) {
    state.containers[containerName] = [];
  }
  // 先从其他容器移除
  removeItemFromAllContainers(itemName);
  // 添加到目标容器(避免重复)
  if (!state.containers[containerName].includes(itemName)) {
    state.containers[containerName].push(itemName);
  }
  saveContainers();
}

// 从容器中移除元素
function removeItemFromContainer(containerName: string, itemName: string) {
  if (state.containers[containerName]) {
    state.containers[containerName] = state.containers[containerName].filter(
...
// 从所有容器中移除元素
function removeItemFromAllContainers(itemName: string) {
  Object.keys(state.containers).forEach(containerName => {
    state.containers[containerName] = state.containers[containerName].filter(
      (name: string) => name !== itemName
    );
  });
...
// 检查元素是否在某个容器中
function isItemInAnyContainer(itemName: string): boolean {
  return Object.values(state.containers).some((items: string[]) => 
    items.includes(itemName)
  );
}

// 获取元素所在的容器名称
function getItemContainer(itemName: string): string | null {
  for (const [containerName, items] of Object.entries(state.containers)) {
    if ((items as string[]).includes(itemName)) {
...
function saveContainers() {
  safeSetItem(STORAGE_CONTAINERS_KEY, JSON.stringify(state.containers));
  dbg('[容器管理] 已保存容器数据:', state.containers);
}

// 清空容器
function clearContainer(containerName: string) {
  if (state.containers[containerName]) {
    state.containers[containerName] = [];
    saveContainers();
    dbg('[容器管理] 已清空容器:', containerName);
  }
}

// 清理无效的容器数据(元素已不存在)
function cleanupContainers() {
  const allElements = state.cachedData.mapElements || [];
  const elementNames = allElements.map((e: any) => e.name);
  
  Object.keys(state.containers).forEach(containerName => {
    // 清理不存在的元素
    state.containers[containerName] = state.containers[containerName].filter(
      (itemName: string) => elementNames.includes(itemName)
    );
    
    // 删除空容器
    if (state.containers[containerName].length === 0) {
...
function renderElementsSidebar() {
  const $list = $('#ci-elements-list');
  $list.empty();
  
  const allElements = state.cachedData.mapElements || [];
  const searchTerm = ($('#ci-elements-search').val() as string || '').toLowerCase();
  
  // 按地点分组
  const locationGroups: any = {};
  allElements.forEach((el: any) => {
    const location = el.location || '未知地点';
    if (!locationGroups[location]) {
      locationGroups[location] = [];
    }
    locationGroups[location].push(el);
  });
  
  // 过滤搜索
  if (searchTerm) {
    Object.keys(locationGroups).forEach(loc => {
      locationGroups[loc] = locationGroups[loc].filter((el: any) => 
        el.name.toLowerCase().includes(searchTerm) ||
        (el.type && el.type.toLowerCase().includes(searchTerm)) ||
        (el.desc && el.desc.toLowerCase().includes(searchTerm))
      );
      if (locationGroups[loc].length === 0) {
        delete locationGroups[loc];
      }
    });
  }
  
  if (Object.keys(locationGroups).length === 0) {
    $list.html('<div style="text-align:center; padding:20px; opacity:0.5;">没有找到元素</div>');
    return;
  }
  
  // 渲染每个地点分组
  Object.keys(locationGroups).sort().forEach(location => {
    const elements = locationGroups[location];
    const $group = $('<div class="ci-location-group"></div>');
    
    // 地点标题
    const $header = $(`
      <div class="ci-location-header">
        <span class="ci-location-name">📍 ${location}</span>
        <span class="ci-location-count">${elements.length}个元素</span>
      </div>
    `);
    
    // 元素列表容器
    const $items = $('<div class="ci-location-items expanded"></div>');
    
    // 渲染每个元素
    elements.forEach((el: any) => {
      const isContainer = isContainerElement(el);
      const inContainer = isItemInAnyContainer(el.name);
      const containerName = inContainer ? getItemContainer(el.name) : null;
      
      let icon = '📦';
      if (isContainer) icon = '📦';
      else if (el.type && el.type.includes('NPC')) icon = '👤';
      else if (el.type && el.type.includes('武器')) icon = '⚔️';
      else if (el.type && el.type.includes('物品')) icon = '🎁';
      
      const $card = $(`
        <div class="ci-element-card ${isContainer ? 'is-container' : ''} ${inContainer ? 'in-container' : ''}" 
             data-element-name="${el.name}" 
             draggable="true">
          <div class="ci-element-header">
            <span class="ci-element-icon">${icon}</span>
            <span class="ci-element-name">${el.name}</span>
            ${el.type ? `<span class="ci-element-type">${el.type}</span>` : ''}
          </div>
          ${el.desc ? `<div class="ci-element-desc">${el.desc}</div>` : ''}
          ${inContainer ? `<div class="ci-element-container-badge">在 ${containerName} 中</div>` : ''}
          ${isContainer && getContainerItems(el.name).length > 0 ? 
            `<div class="ci-element-container-badge">${getContainerItems(el.name).length} 个物品</div>` : ''}
        </div>
      `);
      
      // 拖拽事件
      if (!inContainer) {
        $card.on('dragstart', (e: any) => {
          e.originalEvent.dataTransfer.setData('text/plain', el.name);
          e.originalEvent.dataTransfer.effectAllowed = 'move';
          dbg('[元素拖拽] 开始拖拽:', el.name);
        });
      }
...
// 切换侧边栏显示
function toggleElementsSidebar() {
  const $sidebar = $('#ci-elements-sidebar');
  $sidebar.toggleClass('ci-sidebar-hidden');
  
  if (!$sidebar.hasClass('ci-sidebar-hidden')) {
...
function renderGrid(type: any, $pan: any) {
  ...
  data.forEach((d: any) => {
    ...
    $card.click(e => {
      ...
      // 4. 物品区在编辑模式下的点击（排除扩展按钮和物品格子）
      if ($target.closest('.ci-bag-section').length && state.isEditing) {
        // 只有在编辑模式下，点击物品区才打开编辑弹窗
        // 但要排除点击扩展按钮和物品格子的情况
        if (!$target.closest('.ci-bag-expand-btn, .ci-bag-item').length) {
          e.stopPropagation();
          dbg('[角色卡] 编辑模式下点击物品区');
...
function adjustLayout(locs: any[]) {
  ...
  // 首先应用保存的地点位置
  locs.forEach(l => {
    const savedLayout = state.mapLayout['__location_' + l.name];
    if (savedLayout) {
      l.x = savedLayout.x;
      l.y = savedLayout.y;
      if (savedLayout.width) l.width = savedLayout.width;
      if (savedLayout.height) l.height = savedLayout.height;
      dbg('[地点布局] 应用保存的位置:', l.name, l.x, l.y);
    }
  });
...
function renderMap() {
  try {
    ...
    dbg('[地图渲染] 开始渲染, 编辑模式:', state.isEditing);
    ...
    const mapItems: any[] = [];
    const addedElements = new Set<string>(); // 记录已添加的元素,避免重复
    
    // ==================== 渲染地点 ====================
    adjustedLocs.forEach((l: any) => {
      const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      g.setAttribute('class', 'map-location-group' + (state.isEditing ? ' draggable' : ''));
      ...
      const renderInner = () => {
        ...
        t.setAttribute('class', 'map-location-label');
        t.setAttribute('style', 'font-size: 16px; font-weight: bold; pointer-events:none; fill:var(--map-text);');
      ...
      g.addEventListener('click', e => {
        if (state.isGlobalDragging || state.isEditing) return;
        e.stopPropagation();
        const rect = g.getBoundingClientRect();
        dbg('[地图位置] 点击地点:', l.name);
        showMapPopup(rect.left + rect.width / 2, rect.top + rect.height / 2, [`前往 ${l.name}`]);
      });
      ...
      if (state.isEditing) {
        ...
        resizeHandle.setAttribute('cx', String(l.x + l.width));
        resizeHandle.setAttribute('cy', String(l.y + l.height));
        resizeHandle.setAttribute('r', '6');
        resizeHandle.setAttribute('fill', '#fff');
        resizeHandle.setAttribute('stroke', 'var(--map-accent)');
        ...
        handleDrag(
          ...
          (curr: any) => {
            ...
            const svgX = (curr.clientX - rect.left) * scaleX + vbNow.x;
            const svgY = (curr.clientY - rect.top) * scaleY + vbNow.y;
            
            // 更新尺寸
            l.width = Math.max(100, svgX - l.x);
            l.height = Math.max(60, svgY - l.y);
            renderInner();
          ...
          () => {
            resizing = false;
            state.mapLayout['__location_' + l.name] = { x: l.x, y: l.y, width: l.width, height: l.height };
            saveMapLayout();
            dbg('[地点调整尺寸] 保存:', l.name, 'size:', l.width, 'x', l.height);
          }
        );
        
        // 拖拽移动功能
        let startX = 0, startY = 0, initialX = l.x, initialY = l.y;
        handleDrag(
          $g,
          (startPt: any) => {
            if (resizing) return; // 正在调整尺寸时不移动
          ...
          () => {
            ...
            state.mapLayout['__location_' + l.name] = { x: l.x, y: l.y, width: l.width, height: l.height };
            saveMapLayout();
            dbg('[地点拖拽] 保存地点位置:', l.name, l.x, l.y);
          }
        );
      }
      
      $svg[0].appendChild(g);
      dbg('[地点渲染] 已添加地点到SVG:', l.name, '编辑模式:', state.isEditing, 'class:', g.getAttribute('class'));
      
      // 只收集已经拖拽到地图上的元素(在mapLayout中有记录)
      const locElements = rawElements.filter((e: any) => {
        const elLoc = String(e.location || '').trim();
        const locName = String(l.name).trim();
        const matchesLocation = locName.includes(elLoc) || elLoc.includes(locName);
        const isInContainer = isItemInAnyContainer(e.name);
        const hasLayout = !!state.mapLayout[e.name]; // 只显示有布局记录的元素
        
        return matchesLocation && !isInContainer && hasLayout;
      });
      ...
      locElements.forEach((el: any, i: number) => {
        ...
        // 元素必须有布局记录才显示(通过拖拽添加)
        const layout = state.mapLayout[el.name];
        if (!layout) return;
    ...
    // ==================== 渲染已拖拽到地图的独立元素 ====================
    // 遍历mapLayout中的所有元素,渲染那些已经被用户拖拽到地图上的
    Object.keys(state.mapLayout).forEach(key => {
      // 跳过地点布局
      if (key.startsWith('__location_')) return;
      
      // 查找对应的元素数据
      const el = rawElements.find((e: any) => e.name === key);
      if (!el) return; // 不是元素(可能是角色)
      
      // 避免重复添加(如果已经在地点循环中添加过了)
      if (addedElements.has(el.name)) return;
      
      const isInContainer = isItemInAnyContainer(el.name);
      if (isInContainer) return; // 在容器中的不显示
      
      const layout = state.mapLayout[key];
      const isNPC = ['NPC', '人物', '角色', 'Enemy', 'Boss'].some(t => String(el.type).includes(t));
      const isItem = ['物品', '道具', 'Item', 'Chest'].some(t => String(el.type).includes(t));
      const isContainer = isContainerElement(el);
      ...
      mapItems.push({
        ...
        x: layout.x,
        y: layout.y,
        parent: null, // 独立元素没有父地点
        isNPC,
        isItem,
        isContainer
      });
      
      addedElements.add(el.name);
      dbg('[独立元素] 渲染已拖拽元素:', el.name, '位置:', layout.x, layout.y);
    });
    ...
    mapItems.forEach(item => {
      if (item.type === 'element') {
        const el = item.data;
        const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        g.setAttribute('class', 'map-element-group' + (state.isEditing ? ' draggable' : ''));
        g.setAttribute('data-name', el.name);
        ...
        const renderInner = () => {
          ...
          txt.setAttribute('x', String(item.x + size / 2));
          txt.setAttribute('y', String(item.y + size + 12)); // 矩形下方12px
          txt.setAttribute('text-anchor', 'middle');
          txt.setAttribute('class', 'map-element-label');
          txt.setAttribute('style', 'font-size: 12px; fill: var(--map-text); pointer-events: none;');
          
          // 容器添加特殊标识
          if (item.isContainer) {
            const containerItems = getContainerItems(el.name);
            if (containerItems.length > 0) {
              txt.textContent = `${el.name}(${containerItems.length})`;
              // 添加容器图标
              const icon = document.createElementNS('http://www.w3.org/2000/svg', 'text');
              icon.setAttribute('x', String(item.x + size / 2));
              icon.setAttribute('y', String(item.y + size / 2 + 2));
              icon.setAttribute('text-anchor', 'middle');
              icon.setAttribute('dominant-baseline', 'middle');
              icon.setAttribute('style', 'font-size: 16px; pointer-events: none;');
              icon.textContent = '📦';
              g.appendChild(icon);
            } else {
        ...
        g.addEventListener('click', e => {
          const dist = Math.sqrt((e.clientX - startClickX) ** 2 + (e.clientY - startClickY) ** 2);
          if (dist > 5) return; // 拖拽距离超过5px,不触发点击
          if (state.isEditing) return; // 编辑模式不触发互动
          e.stopPropagation();
          const rect = g.getBoundingClientRect();
          ...
          // 2. 根据类型添加默认选项
          if (item.isNPC) {
            opts.push(`与${el.name}对话`);
            opts.push(`观察${el.name}`);
          } else if (item.isContainer) {
            opts.push(`查看${el.name}`);
            opts.push(`打开${el.name}`);
          } else if (item.isItem) {
            opts.push(`拾取${el.name}`);
            opts.push(`检查${el.name}`);
          } else {
            opts.push(`查看${el.name}`);
            opts.push(`使用${el.name}`);
          }
          
          // 3. 显示弹窗
          if (opts.length === 0) {
            opts.push(`交互${el.name}`);
          }
        ...
        if (state.isEditing) {
          const $g = $(g);
          $g.css('cursor', 'grab');
          let startX = 0, startY = 0, initialX = item.x, initialY = item.y;
          handleDrag(
            $g,
            (startPt: any) => {
              startX = startPt.clientX;
            ...
            () => {
              $g.css('cursor', 'grab');
              state.mapLayout[el.name] = { x: item.x, y: item.y };
              saveMapLayout();
            }
          );
        }
        $svg[0].appendChild(g);
        dbg('[元素渲染] 已添加元素到SVG:', el.name, '编辑模式:', state.isEditing, 'class:', g.getAttribute('class'));
        ...
        const renderInner = () => {
          ...
          if (localImg) {
            ...
            clipCircle.setAttribute('cx', String(item.x + 26));
            clipCircle.setAttribute('cy', String(item.y + 26));
            clipCircle.setAttribute('r', '24');
            clipPath.appendChild(clipCircle);
            defs.appendChild(clipPath);
            gAv.appendChild(defs);
            const circ = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
            circ.setAttribute('cx', String(item.x + 26));
            circ.setAttribute('cy', String(item.y + 26));
            ...
            img.setAttribute('x', String(item.x + 2));
            img.setAttribute('y', String(item.y + 2));
            ...
            tAv.setAttribute('x', String(item.x + 26));
            tAv.setAttribute('y', String(item.y + 26));
        ...
        if (state.isEditing) {
          ...
          handleDrag(
            ...
            () => {
              $gAv.css('cursor', 'grab');
              state.mapLayout[char.name] = { x: item.x, y: item.y };
              saveMapLayout();
            }
          );
        } else {
...
function createUI() {
  try {
    ...
    const $elementsSidebar = $(`
            <div id="ci-elements-sidebar" class="ci-sidebar-hidden">
                <div class="ci-sidebar-header">
                    <div style="display:flex; align-items:center; gap:8px;">
                        <span style="font-size:16px;">📦</span>
                        <span style="font-weight:bold;">元素管理</span>
                    </div>
                    <span class="ci-sidebar-close">${ICONS.close}</span>
                </div>
                <div class="ci-sidebar-search">
                    <input type="text" id="ci-elements-search" placeholder="搜索元素..." />
                </div>
                <div class="ci-sidebar-content" id="ci-elements-list">
                    <div style="text-align:center; padding:20px; opacity:0.5;">加载中...</div>
                </div>
                <div class="ci-sidebar-footer">
                    <button class="ci-sidebar-btn" id="ci-refresh-elements">
                        ${ICONS.refresh} 刷新
                    </button>
                </div>
            </div>
    ...
    $('body').append($con).append($pan).append($mapPan).append($elementsSidebar);
    setTimeout(() => {
...
function bindEvents($con: any, $pan: any, $ops: any, $mapPan: any) {
  ...
  $pan
    ...
    .on('click', function (e: any) {
      ...
      if (state.isEditing) {
        $('.ci-save-layout-btn').show();
        // 进入编辑模式时自动打开侧边栏
        toggleElementsSidebar();
      } else {
        $('.ci-save-layout-btn').hide();
        // 退出编辑模式时关闭侧边栏
  ...
  // 刷新按钮
  $('#ci-refresh-elements').on('click', () => {
    renderElementsSidebar();
    showToast('已刷新元素列表');
  });
  
  // 地图SVG拖放事件 - 使用已存在的$svg变量
  $svg.on('dragover', (e: any) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  $svg.on('drop', (e: any) => {
    ...
    const elementName = e.originalEvent.dataTransfer.getData('text/plain');
    if (!elementName) return;
    
    dbg('[元素拖放] 拖放元素到地图:', elementName);
    
    // 获取SVG坐标
    const svgEl = $svg[0];
    const pt = svgEl.createSVGPoint();
    pt.x = e.originalEvent.clientX;
    pt.y = e.originalEvent.clientY;
    const svgPt = pt.matrixTransform(svgEl.getScreenCTM().inverse());
    
    // 从容器中移除(如果在容器中)
    removeItemFromAllContainers(elementName);
    
    // 设置元素位置(只保存坐标,不保存尺寸)
    state.mapLayout[elementName] = {
      x: svgPt.x,
      y: svgPt.y
    };
    saveMapLayout();
    renderMap();
    renderElementsSidebar(); // 更新侧边栏状态
    showToast(`已将 ${elementName} 添加到地图`);
  });
}
