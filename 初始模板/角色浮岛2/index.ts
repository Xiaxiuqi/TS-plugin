...
const ICONS = {
  ...
  container: `<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M20 8h-3V4H3c-1.1 0-2 .9-2 2v11h2c0 1.66 1.34 3 3 3s3-1.34 3-3h6c0 1.66 1.34 3 3 3s3-1.34 3-3h2v-5l-3-4zM6 18c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm13.5-9l1.96 2.5H17V9h2.5zm-1.5 9c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1z"/></svg>`,
};
  ...
  try {
    ...
    if (elementTable && elementTable.content) {
      ...
      const idx = {
        ...
        parent: getCol(h, ['父元素', '容器', 'Parent', 'Container']),
      ...
      dbg('[地图元素表] 列索引:', idx);
      if (idx.parent === -1) {
        dbg('[地图元素表] ⚠️ 表格中没有"父元素"列,所有元素将正常显示');
      }
      if (idx.name > -1 && idx.loc > -1) {
        elementTable.content.slice(1).forEach((r: any) => {
          if (r[idx.name] && r[idx.loc]) {
            ...
            const parentValue = idx.parent > -1 ? (r[idx.parent] || '') : '';
            ...
            dbg(`[地图元素] 解析: ${r[idx.name]}, 类型: ${r[idx.type] || '物品'}, 父元素: "${parentValue}"`);
          }
        });
      }
    }
...
// 容器弹窗 - 显示容器内的物品列表
function showContainerPopup(x: number, y: number, container: any, childElements: any[]) {
  $('.ci-map-popup-options').remove();
  
  const $popup = $(`
    <div class="ci-container-popup" style="
      position: fixed;
      background: var(--ci-panel-bg, #fff);
      border: 2px solid var(--ci-border, #ccc);
      border-radius: 12px;
      padding: 12px;
      ...
      z-index: 2005;
    ">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px solid var(--ci-border, #ddd);">
        <div style="font-weight: bold; font-size: 14px; color: var(--ci-text, #333);">${ICONS.container} ${container.name}</div>
      </div>
      <div style="font-size: 11px; color: var(--ci-text-dim, #666); margin-bottom: 8px;">
        容器内共有 ${childElements.length} 个物品
      </div>
      <div class="ci-container-items" style="display: flex; flex-direction: column; gap: 4px;"></div>
      ${container.interactions && container.interactions.length > 0 ? `
        <div style="margin-top: 10px; padding-top: 8px; border-top: 1px solid var(--ci-border, #ddd);" class="ci-container-actions"></div>
      ` : ''}
    </div>
  `);
  
  // 填充物品列表
  const $itemsList = $popup.find('.ci-container-items');
  childElements.forEach((item: any) => {
    const $item = $(`
      <div class="ci-container-item" style="
        padding: 6px 8px;
        background: var(--ci-card-bg, #f9f9f9);
        border-radius: 6px;
        cursor: pointer;
        transition: all 0.2s;
        font-size: 12px;
      ">
        <div style="font-weight: 500; color: var(--ci-text, #333);">${item.name}</div>
        ${item.desc ? `<div style="font-size: 10px; color: var(--ci-text-dim, #666); margin-top: 2px;">${item.desc}</div>` : ''}
        ${item.status ? `<div style="font-size: 10px; color: var(--ci-accent, #2196f3); margin-top: 2px;">状态: ${item.status}</div>` : ''}
      </div>
    `);
    ...
    $item.on('click', (ev: any) => {
      ev.stopPropagation();
      const itemActions = item.interactions && item.interactions.length > 0 
        ? item.interactions 
        : [`从${container.name}拾取 ${item.name}`, `检查 ${item.name}`];
      
      // 关闭容器弹窗，显示物品交互选项
      $popup.remove();
      showMapPopup(ev.clientX, ev.clientY, itemActions);
    });
    
    $item.on('mouseenter', function() {
      $(this).css('background', 'var(--ci-hover, #e3f2fd)');
    });
    
    $item.on('mouseleave', function() {
      $(this).css('background', 'var(--ci-card-bg, #f9f9f9)');
    });
  ...
  // 填充容器自身的交互选项
  if (container.interactions && container.interactions.length > 0) {
    const $actions = $popup.find('.ci-container-actions');
    container.interactions.forEach((action: string) => {
      const $actionBtn = $(`
        <div class="ci-option-bubble" style="
          padding: 6px 10px;
          background: var(--ci-accent, #2196f3);
          color: #fff;
          border-radius: 6px;
          cursor: pointer;
          text-align: center;
  ...
  if (left + popupRect.width > winW - 10) {
    left = winW - popupRect.width - 10;
  }
  if (left < 10) left = 10;
  
  if (top + popupRect.height > winH - 10) {
    top = y - popupRect.height - 20;
  }
  if (top < 10) top = 10;
  ...
  // 点击外部关闭
  setTimeout(() => {
    $(document).on('click.container-popup', (e: any) => {
      if (!$(e.target).closest('.ci-container-popup').length && !$(e.target).closest('.ci-map-popup-options').length) {
        $popup.remove();
        $(document).off('click.container-popup');
      }
    });
  }, 100);
}
  ...
  try {
    ...
    adjustedLocs.forEach((l: any) => {
      ...
      g.addEventListener('click', e => {
        ...
        e.stopPropagation();
        const rect = g.getBoundingClientRect(); // 使用g元素本身的坐标
        dbg('[地图位置] 点击地点:', l.name, '坐标:', rect.left, rect.top);
      ...
      const locElements = rawElements.filter((e: any) => {
        ...
        // 只过滤掉明确被收纳进容器的子元素
        // 父元素列必须有实际内容（不是空字符串、null、undefined）才算是子元素
        const parentValue = e.parent;
        const hasParent = parentValue && String(parentValue).trim() !== '' && String(parentValue).trim() !== 'null' && String(parentValue).trim() !== 'undefined';
        const shouldDisplay = !hasParent;
        
        if (matchesLocation) {
          dbg(`[元素过滤] ${e.name}: 位置匹配=${matchesLocation}, 父元素="${parentValue}", 是否有父元素=${hasParent}, 是否显示=${shouldDisplay}`);
        }
        
        return matchesLocation && shouldDisplay;
      });
    ...
    mapItems.forEach(item => {
      if (item.type === 'element') {
        ...
        const renderInner = () => {
          ...
          // 检查是否为容器类型
          const isContainer = ['容器', '储物箱', '箱子', '柜子', '背包', '仓库', 'Container', 'Chest', 'Storage'].some(
            keyword => String(el.type).includes(keyword)
          );
          ...
          if (item.isNPC) {
            ...
            // 容器类型添加右上角徽章
            if (isContainer) {
              const childElements = rawElements.filter((e: any) => {
                const parent = String(e.parent || '').trim();
                return parent === el.name || el.name.includes(parent) || parent.includes(el.name);
              });
              const childCount = childElements.length;
              
              if (childCount > 0) {
                const badge = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
                badge.setAttribute('cx', String(item.x + item.size - 8));
                badge.setAttribute('cy', String(item.y + 8));
                badge.setAttribute('r', '8');
                badge.setAttribute('fill', '#ff5722');
                badge.setAttribute('stroke', '#fff');
                badge.setAttribute('stroke-width', '1');
                elG.appendChild(badge);
                
                const badgeText = document.createElementNS('http://www.w3.org/2000/svg', 'text');
                badgeText.setAttribute('x', String(item.x + item.size - 8));
                badgeText.setAttribute('y', String(item.y + 8));
        ...
        elG.addEventListener('click', e => {
          ...
          if (!state.isEditing) {
            e.stopPropagation();
            const rect = elG.getBoundingClientRect();
            ...
            if (isContainer) {
              // 容器类型：显示子元素列表
              dbg('[地图容器] 点击容器:', el.name);
              const childElements = rawElements.filter((e: any) => {
                const parent = String(e.parent || '').trim();
                return parent === el.name || el.name.includes(parent) || parent.includes(el.name);
              });
              
              dbg('[地图容器] 找到子元素:', childElements.length, '个');
              
              if (childElements.length > 0) {
                // 显示容器内容弹窗
                showContainerPopup(rect.left + rect.width / 2, rect.top + rect.height / 2, el, childElements);
              } else {
                // 容器为空，显示默认交互选项
                const opts = el.interactions && el.interactions.length > 0 
                  ? el.interactions 
                  : [`打开 ${el.name}`, `检查 ${el.name}`];
                showMapPopup(rect.left + rect.width / 2, rect.top + rect.height / 2, opts);
              }
            } else {
              // 普通元素：显示交互选项
              dbg('[地图元素] 点击元素:', el.name, '互动选项:', el.interactions);
              if (el.interactions && el.interactions.length > 0) {
                showMapPopup(rect.left + rect.width / 2, rect.top + rect.height / 2, el.interactions);
              } else {
                const opts = [];
        ...
        const avatarUrl =
          safeGetItem(STORAGE_AVATAR_PREFIX + '主角', '') ||
          safeGetItem(STORAGE_AVATAR_PREFIX + 'Player', '');
        const gAv = document.createElementNS('http://www.w3.org/2000/svg', 'g');
...
async function injectMapTables(): Promise<boolean> {
  ...
  try {
    ...
    const elementTable = {
      ...
      sourceData: {
        note: '记录场景中可交互的实体。支持容器化管理，减少地图元素数量，提升可读性。',
        initNode: '新地点创建时，必须为其添加至少一个地图元素。',
        deleteNode: '实体被消灭/摧毁/取走时删除。物品被收纳进容器后，也应删除该物品的独立元素记录。',
        updateNode: '实体状态因交互改变时更新。容器内的物品发生变化时，更新容器的状态列。',
        insertNode: '场景中出现新的可交互实体时添加。',
        containerTip: '【容器类型说明】元素类型填写"容器"、"储物箱"、"箱子"、"柜子"、"背包"等关键词时，该元素将作为容器显示。其他物品可以通过"父元素"列关联到此容器中。容器的"状态"列建议填写内容物清单（如"内含：铁剑×1、药水×3"），方便用户查看。',
        parentElementTip: '【父元素列】当物品被收纳到某个容器中时，在此列填写容器的名称（如"木箱"、"储物柜"）。填写了父元素的物品将不会直接显示在地图上，而是显示在容器的弹窗列表中。留空表示该元素独立显示在地图上。'
      },
      content: [
        [null, '元素名称', '元素类型', '元素描述', '所属主地点', '状态', '父元素', 'X坐标', 'Y坐标', '宽度', '高度', '互动选项1', '互动选项2', '互动选项3']
      ],
      exportConfig: {}
    };
    ...
    showToast('地图表格已注入，请刷新网页以查看效果', 'info');
    dbg('[地图注入] ========== 完成 ==========');
...
async function removeMapTables(): Promise<boolean> {
  ...
  try {
    ...
    showToast(`已移除${markedKeys.length}个地图表格，请刷新网页以查看效果`, 'info');
    dbg('[地图移除] ========== 完成 ==========');
