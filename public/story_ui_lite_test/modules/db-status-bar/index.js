...
<section class="story-ui-root db-status-bar story-ui-${theme}" data-story-ui-module="${MODULE_ID}">
  ...
  function renderQuestTab(S) {
    ...
    let html = '';
    list.forEach(q => {
      html += `<div class="db-sb-toggle-header" data-db-toggle-next><span>${esc(q.name)} <span style="margin-left:6px;font-size:10px;color:var(--db-gold)">[${esc(q.rating)}]</span> · ${esc(q.progress || '--')} · ${esc(q.client || '--')}</span><span class="db-sb-toggle-icon collapsed">▼</span></div>`;
  ...
  function destroyAvatarModal(modal) {
    if (!modal || modal._dbStatusDisposed) return;
    modal._dbStatusDisposed = true;
    if (modal._cropper) {
  ...
  function removeAvatarModals() {
    document.querySelectorAll('.db-sb-avatar-modal').forEach(modal => {
      if (modal._dbStatusOwner === moduleDefinition) destroyAvatarModal(modal);
    });
  }
  ...
  function showAvatarModal(charName, root) {
    ...
    modal._dbStatusOwner = moduleDefinition;
    modal._dbStatusDisposed = false;
    ...
    function showCropUI(src) {
      loadCropperLib().then(() => {
        if (modal._dbStatusDisposed || !modal.parentNode || !isModuleEnabled()) return;
  ...
  function isModuleEnabled() { return !disposed && ui.registry?.find?.(MODULE_ID) === moduleDefinition && moduleDefinition?.enabled !== false; }
  function isGenerationCurrent(token, root) { return isModuleEnabled() && token === generationToken && Boolean(root?.isConnected); }
  function renderContentNode() { const host=document.createElement('div'); host.innerHTML=renderStatusShell(); return host.firstElementChild; }
  function rerender(root) {
    if (!root?.parentElement || !isModuleEnabled()) return root;
    const next=renderContentNode(); root.replaceWith(next); bindEvents(next); activeDataRoot=next; return next;
  }
  function rerenderAll() { if (isModuleEnabled()) document.querySelectorAll('.db-status-bar[data-story-ui-module="db-status-bar"]').forEach(rerender); }
  async function initData(root) {
    ...
    try {
      if (api?.exportTableAsJson) { const tables=await api.exportTableAsJson(); if (!isGenerationCurrent(token,root)) return; ui.dbStatusData.parseTables(typeof tables==='string'?JSON.parse(tables):tables); }
      else ui.dbStatusData?.loadTestData?.();
    } catch(error) { if(!isGenerationCurrent(token,root))return; console.warn('[db-status-bar] API load failed, using test data:', error); ui.dbStatusData?.loadTestData?.(); }
    if (!isGenerationCurrent(token,root)) return;
    activeDataRoot=rerender(root)||root;
    if (api && registeredTableUpdateApi!==api && typeof api.registerTableUpdateCallback==='function') {
      setDatabaseUpdateHandler(api, () => {
        if (!isModuleEnabled()) return;
        const callbackToken=generationToken;
        if (debounceTimer) clearTimeout(debounceTimer);
        debounceTimer=setTimeout(async()=>{ debounceTimer=null; const target=activeDataRoot; if (!isGenerationCurrent(callbackToken,target)) return;
          try { const tables=await api.exportTableAsJson(); if (!isGenerationCurrent(callbackToken,target)) return; ui.dbStatusData.parseTables(typeof tables==='string'?JSON.parse(tables):tables); activeDataRoot=rerender(target)||target; }
          catch(error) { if(isGenerationCurrent(callbackToken,target))console.error('[db-status-bar] Update failed:', error); }
        },300);
      });
    }
  }
  ...
  function mount(node) { if (!isModuleEnabled()) return; generationToken+=1; const root=node.querySelector?.('.db-status-bar')||node; bindEvents(root); initData(root); }
  function removeMountedDom() { document.querySelectorAll('[data-story-ui-module="db-status-bar"]').forEach(node=>{ const host=node.closest?.('[data-story-ui-after-native-mount="true"], [data-story-ui-raw-mount="true"]')||node; const stack=host.closest?.('[data-story-ui-after-native-stack="true"]'); host.remove(); if(stack&&stack.childElementCount===0)stack.remove(); }); }
