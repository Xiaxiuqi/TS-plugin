...
- 当前版本：`lite_test-0.1.7`

## 模块清单

| 模块 ID            | 名称               | 默认状态 | 说明                                                                                 |
| ------------------ | ------------------ | -------- | ------------------------------------------------------------------------------------ |
| `bp-panel-newvars` | BP战力雷达         | 开启     | 战力面板展示                                                                         |
| `world-log`        | 世界运行报告       | 开启     | 世界状态日志展示                                                                     |
| `manager-ui`       | 管理面板           | 开启     | 前端管理 UI（开关模块、主题切换等）                                                  |
| `db-status-bar`    | 状态栏             | 开启     | 读取状态数据，渲染世界状态、角色、能力、物品和独立任务档案 |
| `db-map`           | 地图               | 开启     | 独立读取地点/地图元素，独占 AI 生成、缓存、SVG 安全清理、交互和自动生成 |
...
- `version`: `releasetest-0.1.1` → 当前 `lite_test-0.1.7`
...
| 项目项               | 状态                                            | 证据                                                                                                                                                                                                                                                                                   | 下一步                                                                                                           |
| -------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| 状态栏/地图模块拆分 | 已修复地图渲染与 AI SVG 提取回归，待酒馆运行时复核 | `db-map` 可从字符串、围栏、实体编码及结构化响应中提取 SVG；缺失根闭合标签时仅在 XML 清洗通过后采用恢复结果；地图通知已迁入面板内部 | 重载 `lite_test-0.1.7` 后确认生成、失败保留旧图、面板通知及主题切换均正常 |
...
- `db-map` 模块提升到 `1.1.2-lite_test`，入口与 loader 缓存版本同步提升到 `lite_test-0.1.7`，并修正 loader 的严格模块版本契约。
- 地图 AI 输出上限提高到 8000 tokens；SVG 提取兼容代码围栏、HTML/数字实体、JSON 字符串及结构化对象返回，同时保留 XML 解析和白名单清洗安全边界。
- 对仅缺少根 `</svg>` 且其余 XML 结构完整的响应尝试恢复；无法安全恢复时保留旧图或数据库基础图，不写入无效缓存。
- 地图过程与错误通知改为面板内窄条，支持昼夜主题、错误 ARIA 语义、自动消失和重渲染状态恢复；仅在地图容器不存在时降级到 `toastr`。
- 修复生成期间主题切换导致 loading overlay 遗留的问题，并通过语法、ESLint、diff 检查及多格式 SVG 表驱动烟测。
...
- 管理模块固定为 `BP战力雷达 → 世界运行报告 → 状态栏 → 地图`，移除 BP 名称中的兼容后缀，并将两个数据库模块的展示名缩短为“状态栏”“地图”。
- 旧版 `db-map=false` 没有来源标识，无法区分是旧逻辑写入还是用户手动关闭；为满足本次恢复地图显示的明确要求，首次升级时将该值重置为默认开启，保留旧 `true`，迁移完成后继续尊重地图独立开关。状态保存失败时不写迁移完成标记，下次加载继续重试。
- 入口与 loader 缓存版本同步提升到 `lite_test-0.1.5`，确保 registry、管理面板和地图模块资源不会继续复用旧缓存。
- 地图管理提示、诊断名称和通知标题同步使用“地图”，避免管理界面继续混用旧名称。
...
- 物品列表保留 200px max-height 内部滚动限制，仅移除卡片内部字段的单行截断；长物品名与卡片字段内容均允许换行完整显示。
