...
> 边界更新（2026-06-18）：本文中将地图描述为状态栏内置功能的部分已被地图/状态栏拆分方案取代。当前实现中，`db-status-bar` 仅负责世界状态、角色、能力、物品、任务和数据库 API；地图数据库解析、AI API、配置、缓存、SVG sanitizer、渲染、交互与自动生成均属于独立 `db-map` 模块。

> 两个模块固定挂载到最后一条 AI 消息末尾，不依赖显式标签；顺序为 `bp-panel-newvars → world-log → db-status-bar → db-map`。
...
- 地图功能不再属于本状态栏设计；其实现与验收以独立 `modules/db-map/` 为准
- 不修改模板 JSON 本身
...
[角色档案面板] ← 可折叠，TAB切换，左右分栏；底部包含独立折叠物品栏
[任务档案面板] ← 独立、默认折叠，不与地图共用工具条或浮层
```
...
- 术式、式神与束缚随当前角色展示在角色档案右栏，不建立第二套功能 TAB。
- 物品栏位于角色档案面板底部，使用独立折叠区，保留 owner/分类筛选、详情和再利用操作。
- 任务位于状态栏底部独立、默认折叠的任务档案面板。

地图开关不会影响物品栏或任务展示，状态栏也不再持有任何地图按钮、缓存或 AI 状态。

### 4.5 独立地图模块边界

地图实现以 `public/story_ui_lite_test/modules/db-map/` 为准：

- `data.js` 独立解析当前位置和地图元素，不复用状态栏 `GameState`。
- `index.js` 独立提供地图 shell、刷新/重绘、缓存、签名、基础地图、详情浮层和自动生成。
- `doMap(root, false)` 用于刷新，优先复用 `mapCache[loc]`。
- `doMap(root, true)` 用于重绘，会清理当前地点缓存并重新生成。
- AI、配置、模型拉取和 SVG sanitizer 仅存在于 `db-map`；状态栏不得调用 AI API。
- 生成结果必须通过 sanitizer，失败时保留旧图或显示数据库基础地图/明确空状态。
- 模块禁用、热替换或 root 失效后，旧异步结果不得写回 DOM 或缓存。
- 地图与状态栏均挂载到最后一条 AI 消息末尾，顺序固定为 `db-status-bar → db-map`，不使用显式标签。

```html
<section class="story-ui-root db-map" data-story-ui-module="db-map">
...
#### 点击交互
`db-map` 监听 `.cm[data-idx]` 点击，在地图根节点内的浮动详情卡片显示元素名称、类型、状态、描述和传说背景。
...
| 文件 | 变更类型 | 说明 |
|---|---|---|
| `modules/db-status-bar/data.js` | 重构 | 只解析状态栏所需世界、角色、能力、物品和任务数据；移除地图元素职责 |
| `modules/db-status-bar/index.js` | 重构 | 保留状态栏渲染、交互、任务档案、头像和数据库更新；移除地图、AI、SVG 与缓存链路 |
| `modules/db-status-bar/style.css` | 修改 | 状态栏内部规则限定 `.db-status-bar`；任务档案保留状态栏视觉；body 头像 portal 独立作用域 |
| `modules/db-map/data.js` | 新增 | 独立解析当前位置和地图元素 |
| `modules/db-map/index.js` | 新增 | 独立地图渲染、AI、缓存、SVG 安全、交互、自动调度和管理接口 |
| `modules/db-map/style.css` | 新增 | 地图完整日夜主题与 `.db-map` 作用域 |
| `preview-db-status.html` | 修改 | loadTestData 调用同步更新 |
| `loader.js` / `index.js` | 修改 | 注册独立模块、固定默认挂载顺序和管理页独立开关 |
| `STATUS_BAR_PLAN.md` | 重写 | 数据映射与拆分后的模块边界对齐当前实现 |
...
### 6.4 地图实现边界
当前已做：地图迁移至独立 `db-map`，包含 SVG 地图、元素详情、无原型缓存、并发 owner、pending 自动请求移交、刷新/重绘、AI 生成、失败保留旧图、基础地图和进入 DOM 前的 `sanitizeSVG()` 白名单清理。状态栏只保留数据库状态展示与任务。

当前未闭环：
- 真实酒馆宿主中的 AI 返回结构、数据库回调行为、快速开关/重载竞态仍需运行时验证。
- 日夜主题、窄屏、浮层边界和拆分前后的视觉一致性仍需浏览器验收。
- 宿主数据库回调未提供注销接口时，模块使用每 API 单桥接回调转发给当前实例；仍需确认宿主 API 对回调生命周期的实际实现。
- 实时动画仍不属于当前交付范围。
...
2. **渲染层**：在 `preview-db-status.html` 中加载模块，目视确认三段式布局、角色左右分栏、底部物品栏与独立任务档案
3. **交互层**：确认角色 TAB 切换、关联能力折叠、物品栏筛选、任务档案和面板折叠均正常工作
...
2. `index.js` — 世界状态薄条 + 角色左右分栏及关联能力 + 底部物品栏 + 独立任务档案
