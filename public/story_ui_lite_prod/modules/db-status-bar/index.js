(() => {
  ...
  const MODULE_VERSION = '1.1.0-lite_prod';
...
<section class="story-ui-root db-status-bar story-ui-${theme}" data-story-ui-module="${MODULE_ID}">
  <main class="db-sb-root">
    ${dataLoadError ? `<div class="db-sb-data-error" role="alert">${esc(dataLoadError)}</div>` : ''}
    ${renderWorldStrip(S)}
  ...
  async function initData(root) {
    ...
    try {
      if (typeof api?.exportTableAsJson !== 'function') {
        ui.dbStatusData?.resetData?.();
        dataLoadError = '数据库接口不可用，无法加载状态数据。';
        console.warn('[db-status-bar] AutoCardUpdaterAPI unavailable; rendering empty state.');
      } else {
      ...
      if(!isGenerationCurrent(token,root))return;
      console.error('[db-status-bar] database load failed; rendering empty state:', error);
      ui.dbStatusData?.resetData?.();
      dataLoadError = `状态数据加载失败：${error?.message || String(error)}`;
    }
    if (!isGenerationCurrent(token,root)) return;
    ...
    if (api && registeredTableUpdateApi!==api && typeof api.registerTableUpdateCallback==='function') {
      setDatabaseUpdateHandler(api, () => {
        ...
        debounceTimer=setTimeout(async()=>{ debounceTimer=null; const target=activeDataRoot; if (!isGenerationCurrent(callbackToken,target)) return;
          try {
            const tables=await api.exportTableAsJson();
            if (!isGenerationCurrent(callbackToken,target)) return;
            ui.dbStatusData.parseTables(typeof tables==='string'?JSON.parse(tables):tables);
            dataLoadError = '';
            activeDataRoot=rerender(target)||target;
          } catch(error) {
            if (!isGenerationCurrent(callbackToken,target)) return;
            console.error('[db-status-bar] Update failed:', error);
            ui.dbStatusData?.resetData?.();
            dataLoadError = `状态数据更新失败：${error?.message || String(error)}`;
            activeDataRoot=rerender(target)||target;
          }
        },300);
      });
