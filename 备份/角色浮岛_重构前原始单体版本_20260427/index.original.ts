...
const STORAGE_RECYCLE_NAME_KEY = 'ci_recycle_name_v1';
const STORAGE_RECYCLE_OPTION_KEY = 'ci_recycle_option_v1';
const STORAGE_RECYCLE_ACTIVE_KEY = 'ci_recycle_active_v1';
...
console.log('%c[浮岛] 脚本加载中... (v1.5 - Release)', 'color: #00ff00; font-weight: bold; font-size: 16px;');
...
const ICONS = {
  ...
  trash: `<svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>`,
...
const state = {
  ...
  isSkillsOpen: false, // 技能面板开关
  isInventoryMenuOpen: false, // 物品/技能菜单开关
...
function syncPanelPosition($con: any, $pan: any, islandLeft: number, islandTop: number) {
  ...
  // 限制范围：不再限制最大宽度，允许用户自由调整；最小宽度由CSS或内容决定，这里只确保不为负数
  if (availableW < 0) availableW = 0;

  // 应用最大宽度 (实现挤压效果)
  $pan.css('max-width', availableW + 'px');

  // [Fix] 如果当前宽度超过了可用空间，强制更新 width，防止从边缘拉远时弹回原来的大尺寸
  // 同时也解决了在窄屏模式下，因为 width 仍然很大导致拖拽调整宽度失效的问题
  if ($pan.outerWidth() > availableW) {
    $pan.css('width', availableW + 'px');
  }

  // 获取应用 max-width 后的实际宽度
...
// ========== 数据库模板存取辅助函数 (V1.3.0 适配 Extension Settings) ==========
const ACU_DB_NAMESPACE = 'shujuku_v100__userscript_settings_v1';
const ACU_DB_GLOBAL_META_KEY = 'shujuku_v100_globalMeta_v1';
const ACU_DB_PROFILE_PREFIX = 'shujuku_v100_profile_v1';
...
function saveTavernExtensionSettings() {
  const win = window as any;
...
/**
 * 获取数据库模板（优先读取 Extension Settings，兼容 LocalStorage）
 */
function getDatabaseTemplate_ACU(): { key: string; content: string; source: 'extension' | 'local' } | null {
  ...
  if (settings && settings.__userscripts && settings.__userscripts[ACU_DB_NAMESPACE]) {
    ...
    if (ns[key]) {
      ...
      return { key: key, content: ns[key], source: 'extension' };
    }
  }
 ...
 * 保存数据库模板
 */
function saveDatabaseTemplate_ACU(key: string, content: string, source: 'extension' | 'local'): boolean {
  if (source === 'extension') {
    const settings = getTavernExtensionSettings();
    if (settings) {
...
async function forceDeleteMapTemplate(): Promise<boolean> {
  try {
    ...
    if (templateInfo) {
      ...
      dbg('[强制删除] 找到模板:', templateKey, '来源:', source);
...
function processData(rawData: any) {
  ...
  try {
    ...
    // 新增：如果未找到位置，尝试从所有包含'全局数据'的表中提取
    if (!result.protagonistLoc) {
      // 遍历所有表，寻找表名包含'全局数据'的表
    ...
    Object.values(processedData).forEach((s: any) => {
      ...
      if (tableName.includes('选项')) {
        console.log('[选项提取] 发现选项表:', tableName);
        // 遍历所有行（跳过表头）
    ...
    // 收集所有包含角色信息的表名（即使是被排除的表，如果被用于左侧显示，也应该在右侧显示）
    // 这里我们先扫描一遍所有表，标记哪些表被用于左侧信息提取
    ...
    Object.values(processedData).forEach((s: any) => {
      ...
      // 排除逻辑优化：如果表在排除列表中，但被标记为左侧信息源，则不排除
      ...
      // 特殊处理：将'全局'表设为白名单（如果它包含姓名列，说明它可能包含角色信息）
      ...
      h.forEach((colName: string, colIdx: number) => {
        ...
        // 其他信息列（排除已处理的列）
          ...
          // 额外检查：如果该列已经被归类为身体状态列，则不作为其他信息显示
          // 修改：允许包含"地点/位置"的列，以便从全局表中提取主角位置信息到右侧显示
    ...
    Object.values(processedData).forEach((s: any) => {
      ...
      if (isProtagTable || isImportantTable || isCompanionTable) {
        rows.forEach((r: any, rowIdx: number) => {
          ...
          // 优化状态标签提取逻辑：统一使用搜索+排除策略
          // 1. 找到所有符合要求的状态列
          const statusCols = h
            .map((col, i) => ({ col, i }))
            .filter(({ col }) => col && ['特殊状态', '状态', 'BUFF'].some(kw => col.includes(kw)));
          ...
          if (statusCols.length > 0) {
            ...
            // 排除逻辑：只有当存在多列状态时，才排除已在左侧显示的列
            if (statusCols.length > 1 && hasLeftSideStatus) {
              const leftSideLabels = extraInfo.bodyStatus.map((s: any) => s.label);
              // 过滤掉列名完全匹配的列，确保不重复显示已在左侧展示的状态
              colsToUse = statusCols.filter(sc => !leftSideLabels.includes(sc.col));
            }
    ...
    // [Fix] 确保主角获取全局位置信息
    // 如果主角的位置在角色表中未定义或为'未知'，但从全局表中提取到了位置，则回填
    if (result.main.length > 0 && result.protagonistLoc) {
      // 优先查找来自主角信息表的角色，否则取第一个主要角色
      const protagonist =
      ...
      if (protagonist && (protagonist.loc === '未知' || !protagonist.loc || protagonist.loc === '')) {
        ...
        // 同时更新缓存的额外信息中的位置（如果有）
        if (result.charExtraInfo && result.charExtraInfo[protagonist.name]) {
          // 这里不直接修改charExtraInfo结构，因为loc是主属性
        }
      }
    }
  } catch (e) {
...
function showArchiveEditDialog(d: any) {
  ...
  if (charExtra.stats && charExtra.stats.length > 0) {
    ...
    archiveEditHtml += (charExtra.stats || [])
      .map(
        (stat: any, idx: number) => `
        <div class="ci-archive-edit-item" data-type="stats" data-idx="${idx}" data-src-table="" data-src-col="">
...
function sendTakeActionMessage(items: any[], protagonistCore: string) {
  ...
  if ($textarea.length) {
    ...
    try {
      ...
      console.warn('Failed to set context input:', e);
    }
  }
  showToast('已生成拿取动作描述');
}
...
function handleDiscardItem(item: any) {
  ...
  // 找出除了主角以外的拥有者
  const otherOwners = rawOwners.filter(n => extractCore(n) !== pCore);
  ...
  if (otherOwners.length === 0) {
    ...
    const presentNames = getPresentCharacterList();
    // 只要有一个目标在场，就视为在场
    const isAnyPresent = otherOwners.some(n => presentNames.has(extractCore(n)));

    if (isAnyPresent) {
      message = `${pCore}与${targetNamesStr}商量了一下，将${item.name}丢弃了。`;
    } else {
      // 都不在场，查找第一个主要目标的位置
      const firstTargetCore = extractCore(otherOwners[0]);
      const allChars = [...state.cachedData.main, ...state.cachedData.side, ...state.cachedData.retired];
      const targetChar = allChars.find(c => extractCore(c.name) === firstTargetCore);
      const location = targetChar && targetChar.loc && targetChar.loc !== '未知' ? targetChar.loc : null;
...
function renderLatestNews(worldInfo: any): any {
  ...
  // 逻辑去重：如果 matchedOutlineText 存在，则摘要部分(outlineText)不再显示，避免冗余
...
// ========== 物品仓库面板渲染 ==========
function renderInventoryPanel($container: any, viewMode: 'character' | 'warehouse' | 'recycle' = 'character') {
  $container.empty();
  ...
  const itemFound =
    storedRecycleName && allItems.some(i => i.name.includes(storedRecycleName) || storedRecycleName.includes(i.name));
  ...
  if (itemFound) {
    ...
    // 如果 isActive 为 false，说明还在等待物品出现，不重置
  }
...
// 渲染再利用视图
function renderRecycleView($container: any, allItems: any[], allChars: any[]) {
  ...
  // 绑定销毁按钮事件
  $actionBar.find('.ci-destroy-btn').on('click', function (e: any) {
    e.stopPropagation();
    if ($(this).hasClass('active')) {
      ...
      $mid.find('.ci-destroy-actions').css('display', 'flex'); // Ensure flex for gap to work
  ...
  // 绑定合成按钮事件
  $actionBar.find('.ci-synthesize-btn').on('click', function (e: any) {
    e.stopPropagation();
    if ($(this).hasClass('active')) {
      resetRecycleUI();
    } else {
      ...
      $mid.find('.ci-synthesize-actions').css('display', 'flex'); // Ensure flex for gap to work
      $mid.css({ display: 'flex', opacity: 0 }).animate({ opacity: 1 }, 200);
  ...
  // 渲染仓库视图内容 (复用仓库视图的分类逻辑，但需要适配选择模式)
...
// ========== 技能面板渲染 (V1.3.0) ==========
function renderSkillsPanel($container: any, viewMode: 'character' | 'skill' = 'character') {
  $container.empty();
...
function renderCharacterSkillsView($container: any, allSkills: any[], allChars: any[]) {
  const protagonistName = getProtagonistName();
  const extractCore = (n: string) => {
    if (!n) return '';
    const m = String(n)
...
function renderAllSkillsView($container: any, allSkills: any[]) {
  ...
  $categoryTabs.on('click', '.ci-inv-cat-tab', function (e: any) {
    ...
    // 使用 siblings() 确保只移除同级标签的 active 类
...
function createSkillCard(skill: any) {
  // 技能图标逻辑：这里简单使用工具图标，也可以根据类型选择不同图标
...
function showSkillDetail(skill: any, event: any) {
  $('.ci-item-card-popup').remove();
...
function openSkillEditModal(skill: any) {
...
async function saveSkillData(originalSkill: any, newData: any) {
  const api = (window as any).AutoCardUpdaterAPI || (window.parent as any).AutoCardUpdaterAPI;
  if (!api || !api.exportTableAsJson || !api.importTableAsJson) {
    showToast('无法连接到数据库API', 'error');
    return;
...
function handleUseSkill(skill: any) {
  const protagonistName = getProtagonistName();
  const extractCore = (n: string) => {
    const m = String(n)
      .trim()
  ...
  const ownerCore = extractCore(owner);

  let message = '';

  // 判断是否为主角技能
  // 简单判断：如果 owner 包含 "主角" 或者等于 protagonistName
  ...
  if (isProtagonist) {
    ...
    // 非主角技能逻辑
    const allChars = [...state.cachedData.main, ...state.cachedData.side, ...state.cachedData.retired];
    const ownerChar = allChars.find(c => extractCore(c.name) === ownerCore);
...
function handleDestroyConfirm(items: any[], recycleName: string) {
  const protagonistName = getProtagonistName();
  const extractCore = (n: string) => {
    const m = String(n)
      .trim()
  ...
  items.forEach(item => {
    ...
    rawOwners.forEach(owner => {
      const coreName = extractCore(owner);
      if (coreName !== pCore) {
  ...
  if (othersList.length === 0) {
    ...
    const presentNames = getPresentCharacterList();
    const presentOthers = othersList.filter(name => presentNames.has(name));
    ...
    if (absentOthers.length === 0) {
      ...
      if (othersList.length === 1) {
        ...
        // 查找位置
        const allChars = [...state.cachedData.main, ...state.cachedData.side, ...state.cachedData.retired];
        const targetChar = allChars.find(c => extractCore(c.name) === targetName);
        const location = targetChar && targetChar.loc && targetChar.loc !== '未知' ? targetChar.loc : null;
...
function handleSynthesizeConfirm(items: any[], recycleName: string) {
  const protagonistName = getProtagonistName();
  const extractCore = (n: string) => {
    const m = String(n)
      .trim()
  ...
  if ($textarea.length) {
    ...
    try {
      ...
      console.warn('Error parsing globalMeta:', e);
    }
  }
  showToast('已生成合成描述');
}

function showRecycleModal($container: any) {
  ...
  const validate = () => {
    ...
    if (val && checked) {
      $saveBtn
        ...
        .css({ background: 'linear-gradient(135deg, #4caf50 0%, #43a047 100%)', cursor: 'pointer' });
    } else {
  ...
  $saveBtn.on('click', function () {
    ...
    if ($textarea.length) {
      ...
      try {
        ...
        // Ignore storage errors in parent
      }
    }
 ...
 * 生成SVG雷达图
 */
function generateRadarChart(stats: { label: string; value: number; displayValue?: string }[]) {
  if (!stats || stats.length < 3) return ''; // 至少需要3个属性才能构成多边形

  const size = 200; // SVG画布大小
  const center = size / 2;
  const radius = size / 2 - 40; // 留出文字空间
  const levels = 4; // 网格层数

  // 动态计算最大值：基础50，每超出50增加一档
  ...
  const getCoord = (value: number, index: number, total: number) => {
    const angle = (Math.PI * 2 * index) / total - Math.PI / 2; // -PI/2 从上方开始
  ...
  stats.forEach((s, i) => {
    ...
    labelsHtml += `
      <g class="radar-label-group" data-stat-name="${s.label}" style="cursor: pointer;">
          <text x="${lx}" y="${ly}" text-anchor="${anchor}" fill="rgba(255,255,255,0.8)" font-size="12" dy="-5">${s.label}</text>
          <text x="${lx}" y="${ly}" text-anchor="${anchor}" fill="var(--smart-theme-color, #00e5ff)" font-size="10" dy="8">${displayVal}</text>
      </g>
    `;
  });
  ...
  return `
    <div class="radar-chart-container">
      <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" class="radar-chart-svg">
        <g class="radar-grids">${gridHtml}</g>
        <g class="radar-axes">${axesHtml}</g>
        <polygon points="${dataPoints}" fill="rgba(0, 229, 255, 0.3)" stroke="var(--smart-theme-color, #00e5ff)" stroke-width="2" class="radar-area"/>
 ...
 */
// --- Avatar Upload & Cropper Logic Start ---

let isCropperLoaded = false;

function loadCropperLibrary(callback: () => void) {
...
function createAvatarSelectionModal(d: any) {
  ...
  $modal.find('.ci-url-confirm-btn').on('click', e => {
    ...
    fetch(url)
      ...
      .catch(() => {
        // Fallback check: Try loading without CORS to see if it's a CORS issue or invalid URL
...
function createCropperModal(imageSrc: string, d: any) {
  ...
  const initCropper = () => {
    ...
    if (Cropper && image) {
      ...
      cropper = new Cropper(image, {
        aspectRatio: 1,
        checkOrientation: false, // Prevent XHR CORS issues
        viewMode: 1,
        dragMode: 'move',
        autoCropArea: 1,
        restore: false,
...
// --- Avatar Upload & Cropper Logic End ---
...
function createSettingsUI() {
  try {
    ...
    const $header = $(
      `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-shrink:0;"><span style="font-weight:bold;font-size:16px;">浮岛设置 <span style="font-size:10px;font-weight:normal;opacity:0.6;">v1.5</span></span><span class="ci-close-btn">${ICONS.close}</span></div>`,
    );
    ...
    `);
    const $mapContent = $mapSection.find('.ci-accordion-content');

    const $mapSec = $(
      `<div class="ci-settings-section"><div class="ci-settings-title">地图功能<span class="ci-experimental-tag">实验性功能</span><span style="color:#f44336;font-size:12px;margin-left:8px;font-weight:normal;">(不推荐使用)</span></div></div>`,
    );
    const $mapToggle = $(`
    ...
    // 开关事件
    $mapToggle.find('#ci-map-toggle').on('change', async function (e) {
      e.stopPropagation();
...
只会删除带"_浮岛地图_"标记的表格，不会影响您自己创建的表格。`)
    ...
    const $mapNotice = $(`
      <div class="ci-map-notice">
        <div class="ci-notice-title">地图功能相关注意事项（不建议使用）：</div>
        <div class="ci-notice-content">
          地图注入属于修改模板行为，操作过程需要删除当前聊天的本地数据。注入功能不会影响您的模板，但为了安全考虑，请导出自己的模板以防万一。（建议自行利用缝合模板脚本进行模板缝合，不建议直接使用该功能。）
        </div>
        <div class="ci-notice-steps">
          <div class="ci-step">
            <span class="ci-step-number">1</span>
            <div class="ci-step-content">开启启用地图表格功能，将会自动帮您注入地图模板，还需要您进行一段手动操作：请手动前往【数据库】——【数据管理】——【打开可视化表格编辑器】点击右上角【保存到通用模板】并在【数据管理界面，手动点击【删除所有本地数据】；完成这部分操作后，您可以进行手动填表，地图功能会正常运转</div>
          </div>
          <div class="ci-step">
    ...
    $mapContent.append($mapSec).append($mapNotice);
    $accordion.append($mapSection);

    // 3. 性能设置 (Performance Section)
    const $perfSection = $(`
      <div class="ci-accordion-item">
        <div class="ci-accordion-header">
          <span>性能设置</span>
    ...
    // 开关事件
    $relationToggle.find('#ci-relation-auto-redraw').on('change', function (e) {
      e.stopPropagation();
      const isChecked = $(this).prop('checked');
    ...
    // 手风琴交互逻辑
    $accordion.on('click', '.ci-accordion-header', function () {
      const $header = $(this);
...
async function injectMapTables(): Promise<boolean> {
  ...
  try {
    ...
    const { key: templateKey, content: templateStr, source } = templateInfo;
    dbg('[地图注入] ✅ 读取到模板:', templateKey, '来源:', source);
...
async function removeMapTables(): Promise<boolean> {
  ...
  try {
    ...
    if (!templateInfo) {
      ...
      const { key: templateKey, content: templateStr, source } = templateInfo;
      dbg('[地图移除] 读取到模板:', templateKey, '来源:', source);
...
function toggleEditMode() {
  ...
  if (state.isSkillsOpen) {
    ...
    renderSkillsPanel($skillsContent, viewMode);
  }
}
...
function bindEvents($con: any, $pan: any, $ops: any, $mapPan: any) {
  ...
  $('body').on('click', '#ci-panel .ci-pin-btn', function (e: any) {
    ...
    // [Fix] 固定时清除 max-width 限制，允许用户自由调整宽度
    if (newPinnedState) {
      $panel.css('max-width', '');
    }
    ...
    .each(function () {
      ...
      handleDrag(
        ...
        (start: any, e: any) => {
          ...
          // [Fix] 拖动开始时清除 max-width，防止残留的挤压限制影响拖动后的调整
  ...
  // 物品菜单按钮点击事件
  $('.ci-menu-bubble-btn').on('click', function (e: any) {
    e.stopPropagation();
    ...
    if (target === 'inventory') {
      ...
      if (!state.isSkillsOpen) {
        ...
        $('.ci-btn[data-type="inventory"]').addClass('active');
        renderSkillsPanel($('#ci-skills-panel .ci-skills-content'));
      }
    }
  });
  ...
  // 属性图标签点击事件 (v1.36 新增)
  $('body').on('click', '.radar-label-group', function (e: any) {
    e.stopPropagation();
    const $this = $(this);
    ...
    $bubble.css({
      top: rect.top - 30 + 'px',
      left: rect.left + rect.width / 2 - 20 + 'px', // 居中
    });
  ...
  $('#ci-hidden-input').on('change', function (e: any) {
    ...
    reader.onload = (evt: any) => {
      ...
      if (state.currentUploadChar) {
        ...
        // Load cropper and show modal
        loadCropperLibrary(() => {
          createCropperModal(res, state.currentUploadChar);
        });
      }
    };
  ...
  const globalClickHandler = (e: any) => {
    // [Fix] 优先检查拖动状态，防止拖动结束时的点击事件误触发关闭逻辑
    ...
    if (state.isOptionsOpen) {
      ...
      // [Fix] 关闭选项气泡后立即停止，防止同时触发收起浮岛逻辑
      if (e && e.stopPropagation) e.stopPropagation();
      return;
    }

    // 1.5 物品菜单处理
    if ($target.closest('.ci-inventory-menu').length || $target.closest('.ci-btn[data-type="inventory"]').length) {
      return;
    }
    if (state.isInventoryMenuOpen) {
      state.isInventoryMenuOpen = false;
    ...
    const isSubPanelOpen = state.isRelationOpen || state.isInventoryOpen || state.isWorldInfoOpen || state.isSkillsOpen;
  ...
  $(document)
    ...
    .on('click.ci_sub_close', '.ci-sub-panel .ci-close-btn', function (e: any) {
      e.stopPropagation();
      e.preventDefault();
  ...
  $(document)
    ...
    .on('click.ci_sub_pin', '.ci-sub-panel .ci-pin-btn', function (e: any) {
      e.stopPropagation();
      const $btn = $(this);
  ...
  $(document)
    .off('click.ci_rel_bg')
    .on('click.ci_rel_bg', '#ci-relation-panel', function (e: any) {
      const $target = $(e.target);
  ...
  $(document)
    .off('click.ci_inv_bg')
    .on('click.ci_inv_bg', '#ci-inventory-panel', function (e: any) {
      const $target = $(e.target);
  ...
  $(document)
    ...
    .on('click.ci_skills_bg', '#ci-skills-panel', function (e: any) {
      const $target = $(e.target);
      ...
      if ($target.is('#ci-skills-panel') || $target.is('.ci-skills-content') || $target.is('.ci-panel-content')) {
        $panel.removeClass('visible');
        state.isSkillsOpen = false;
        if (!state.isInventoryOpen) {
          $('.ci-btn[data-type="inventory"]').removeClass('active');
  ...
  $(document)
    ...
    .on('click.ci_world_bg', '#ci-worldinfo-panel', function (e: any) {
      const $target = $(e.target);
  ...
  if (api && api.exportTableAsJson) {
    ...
    allTables.forEach((table: any) => {
      ...
      if (isOutlineTable) {
        ...
        const outlineIdx = getColHelper(h, ['概览', '大纲', '概要', '计划']);
        const indexIdx = getColHelper(h, ['索引', '编码', '编号', '编码索引']);
        const timeIdx = getColHelper(h, ['时间', '时间跨度', '日期']);
        ...
        if (summaryIdx > -1) {
          rows.forEach((row: any) => {
            ...
            if (rowIndex == targetIndex && rowTime == targetTime) {
              summaryTable = table;
              summaryRow = row;
              // 如果纪要表中也包含大纲列，则同时作为大纲来源（除非已经找到了独立的大纲表且匹配）
              if (outlineIdx > -1 && row[outlineIdx] && !outlineRow) {
                outlineTable = table;
                outlineRow = row;
              }
            }
...
function syncInventoryMenuPosition(islandLeft: number, islandTop: number) {
  ...
  // 计算目标垂直中心点 (基于传入的 islandTop + 相对偏移)
