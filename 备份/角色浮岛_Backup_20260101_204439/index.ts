...
console.log('%c[浮岛] 脚本加载中... (v1.1 - Release)', 'color: #00ff00; font-weight: bold; font-size: 16px;');
...
function applyOpacity(type: 'main', val: number) {
  ...
  // 对于已经显示的元素，直接设置透明度
  const mainSelectors =
    '#ci-island-container, #ci-panel.visible, #ci-relation-panel.visible, #ci-inventory-panel.visible, #ci-worldinfo-panel.visible, .ci-options-container, .ci-settings-overlay, .ci-character-popup, .ci-item-card-popup:not(.ci-map-location-popup)';
...
function processData(rawData: any) {
  ...
  try {
    ...
    Object.values(processedData).forEach((s: any) => {
      if (!s || !s.name) return;
      // 增强表名匹配：包含 '物品', '背包', '道具', '库存', '装备'
      const isItemTable = /物品|背包|道具|库存|装备/.test(s.name);
...
function renderRelationGraph($container: any) {
  ...
  // 2. 创建节点 (初始位置)
  nodes.push({ id: protagonist.name, data: protagonist, isProtagonist: true, x: 0, y: 0, vx: 0, vy: 0 });
  ...
  otherChars.forEach((char, i) => {
    const angle = (i / otherChars.length) * Math.PI * 2;
  ...
  // 3. 解析连线并分组 (优化：合并相同关系的连线)
  // Key: sourceId_label (用于合并相同关系的连线)
  ...
  allChars.forEach(char => {
    ...
    parts.forEach((part: string) => {
      ...
      let displayLabel = part; // 用于显示的标签
      let foundExplicitTarget = false;

      // 1. 尝试解析 [Target]Label 格式
      const matchBracket = part.match(/^\[(.*?)\](.*)$/);
      if (matchBracket) {
        targetName = matchBracket[1];
        displayLabel = part.replace(/\[|\]/g, '').trim(); // 过滤掉[]符号，只显示内容
        foundExplicitTarget = true;
      } else {
        // 2. 尝试匹配以已知角色名开头的情况 (例如 "A的契约者")
        for (const name of allCharNames) {
          if (name === char.name) continue; // 跳过自己

          // v8.8修复：使用includes代替startsWith，提高识别率；保留完整标签不截断
          if (part.includes(name)) {
            targetName = name;
            displayLabel = part; // 用户要求显示完全的【A的契约者】
            foundExplicitTarget = true;
            break;
          }
        }
      ...
      // 如果没有找到明确的目标，默认指向主角，displayLabel 保持原样
      if (!foundExplicitTarget) {
        targetName = protagonist.name;
        displayLabel = part;
      }

      // v8.8修复：不再自动去除开头的"的"，防止破坏标签完整性
      // if (displayLabel.startsWith('的')) {
      //   displayLabel = displayLabel.substring(1).trim();
      // }

      if (!displayLabel) displayLabel = '关系'; // 防止空标签
      ...
      if (targetNode && targetNode.id !== char.name) {
        ...
        // 分组键：源ID + 显示标签
        // 这样 A->B (同学) 和 A->C (同学) 会被分到一组
        const groupKey = `${char.name}_${displayLabel}`;
        ...
        // 避免重复添加同一个目标
        if (group && !group.targets.includes(targetNode.id)) {
          group.targets.push(targetNode.id);
  ...
  // 4.1 绘制连线 (新版：直角折线 + 分支合并 + 标签方向优化)

  // 辅助函数：绘制带标签的连线
  const drawLabel = (x: number, y: number, text: string, isVertical: boolean = false, offset: number = 0) => {
    const labelBg = document.createElementNS(svgNS, 'rect');
    const labelText = document.createElementNS(svgNS, 'text');
    labelText.textContent = text;

    // 根据偏移量调整位置（用于双向关系不重叠）
    // 如果是垂直文字，偏移是在X轴；如果是水平文字，偏移是在Y轴
    let offsetX = isVertical ? offset : 0;
    let offsetY = isVertical ? 0 : offset;

    let finalX = x + offsetX;
    let finalY = y + offsetY;

    // 检查标签是否会与节点重叠，如果重叠则调整位置
    const nodeRadius = 35; // 节点半径
    for (const node of Object.values(nodeMap)) {
      const nodeObj = node as any;
      const distance = Math.sqrt(Math.pow(finalX - nodeObj.x, 2) + Math.pow(finalY - nodeObj.y, 2));
      if (distance < nodeRadius + 20) {
        // 20px缓冲区
        // 根据文字方向调整标签位置，避免重叠
        if (isVertical) {
          // 垂直文字，向左右方向调整
          const direction = finalX < nodeObj.x ? -1 : 1;
          offsetX += direction * 30;
        } else {
          // 水平文字，向上或向下调整
          const direction = finalY < nodeObj.y ? -1 : 1;
          offsetY += direction * 20;
        }
        finalX = x + offsetX;
        finalY = y + offsetY;
        break; // 只处理第一个重叠的节点
    ...
    if (isVertical) {
      ...
      const textW = 18;
      const textH = Math.max(24, charCount * 14 + 8);
      ...
      const textW = Math.max(24, Math.min(textWidth + 12, 250));
      const textH = hasChinese ? 18 : 16;
      labelBg.setAttribute('x', String(finalX - textW / 2));
      labelBg.setAttribute('y', String(finalY - textH / 2));
      labelBg.setAttribute('width', String(textW));
      labelBg.setAttribute('height', String(textH));
  ...
  // 辅助函数：生成直角路径 (Manhattan Path)
  // type: 'L' (L型), 'Z' (Z型/S型), 'U' (U型)
  // offset: 路径偏移量，用于避免重叠
  const getManhattanPath = (x1: number, y1: number, x2: number, y2: number, offset: number = 0) => {
    const dx = x2 - x1;
    const dy = y2 - y1;
    ...
    // 简单的避障策略：如果直线路径上可能有节点（即dx或dy很小），则强制绕行
    // 节点大小约80，所以如果坐标差小于80，说明在同一直线/列上

    if (absDx > absDy) {
      // 横向距离更远，优先横向走
      // 路径：(x1, y1) -> (midX, y1) -> (midX, y2) -> (x2, y2)
      const midX = (x1 + x2) / 2 + offset; // 偏移加在中间竖线上
      path = `M ${x1} ${y1} L ${midX} ${y1} L ${midX} ${y2} L ${x2} ${y2}`;

      // 标签放在最长的那段
      if (Math.abs(midX - x1) > Math.abs(y2 - y1)) {
        // 第一段横线更长
        labelX = (x1 + midX) / 2;
        labelY = y1;
        isVertical = false;
      } else if (Math.abs(y2 - y1) > Math.abs(x2 - midX)) {
        // 中间竖线更长
        labelX = midX;
        labelY = (y1 + y2) / 2;
        isVertical = true;
      ...
      // 纵向距离更远，优先纵向走
      // 路径：(x1, y1) -> (x1, midY) -> (x2, midY) -> (x2, y2)
      const midY = (y1 + y2) / 2 + offset; // 偏移加在中间横线上
      path = `M ${x1} ${y1} L ${x1} ${midY} L ${x2} ${midY} L ${x2} ${y2}`;

      // 标签放在最长的那段
      if (Math.abs(midY - y1) > Math.abs(x2 - x1)) {
        // 第一段竖线更长
        labelX = x1;
        labelY = (y1 + midY) / 2;
        isVertical = true;
      } else if (Math.abs(x2 - x1) > Math.abs(y2 - midY)) {
        // 中间横线更长
        labelX = (x1 + x2) / 2;
        labelY = midY;
        isVertical = false;
    ...
    return { path, labelX, labelY, isVertical };
  };

  const labelElements: Element[] = [];

  relationGroups.forEach(group => {
    const { sourceNode, targetNodes, label } = group;
    ...
    const strokeColor = `hsl(${hue}, 40%, 60%)`; // 稍微深一点，增加对比度

    if (targetNodes.length > 1) {
      // === 分支合并 (One-to-Many) - 直角版 ===

      // 1. 计算 Hub 点
      // 策略：找到所有 Target 的包围盒中心，然后从 Source 出发，
      // 走一段距离作为主干，然后分叉。
      ...
      // Hub 点位置：在 Source 和 Center 之间，靠近 Source 的地方
      // 这样主干比较短，分支比较长，看起来像树
      const dx = centerX - x1;
      const dy = centerY - y1;
      ...
      if (Math.abs(dx) > Math.abs(dy)) {
        // 横向主干
        const dirX = dx > 0 ? 1 : -1;
        const stemLen = Math.min(100, Math.abs(dx) * 0.5); // 主干长度
        hubX = x1 + stemLen * dirX;
        hubY = y1;
        mainPathD = `M ${x1} ${y1} L ${hubX} ${hubY}`;
        labelX = (x1 + hubX) / 2;
        labelY = y1;
        isVertical = false;
      } else {
        // 纵向主干
        const dirY = dy > 0 ? 1 : -1;
        const stemLen = Math.min(100, Math.abs(dy) * 0.5);
        hubX = x1;
        hubY = y1 + stemLen * dirY;
        mainPathD = `M ${x1} ${y1} L ${hubX} ${hubY}`;
        labelX = x1;
        labelY = (y1 + hubY) / 2;
        isVertical = true;
      ...
      // 绘制主干标签
      labelElements.push(drawLabel(labelX, labelY, label, isVertical));

      // 绘制分支 (Hub -> Targets) - 使用直角折线
      targetNodes.forEach(target => {
        // 从 Hub 到 Target 的直角路径
        // 为了美观，分支线稍微细一点
        const { path } = getManhattanPath(hubX, hubY, target.x, target.y, 0);

        const branchPath = document.createElementNS(svgNS, 'path');
        branchPath.setAttribute('d', path);
        branchPath.setAttribute('stroke', strokeColor);
        branchPath.setAttribute('stroke-width', '1.5');
        branchPath.setAttribute('fill', 'none');
        mainGroup.appendChild(branchPath);
      });
    } else {
      // === 单对单 (One-to-One) - 直角版 ===
      ...
      // 检查双向关系
      const isBidirectional = rawLinks.some(l => l.source === target.id && l.target === sourceNode.id);

      // 偏移量计算
      // 如果是双向关系，我们需要把两条线分开
      // 偏移量基于 Source 和 Target 的 ID 比较，确保确定性
      let offset = 0;
      let labelOffset = 0;
      ...
      if (isBidirectional) {
        ...
        // A->B: offset = -10
        // B->A: offset = +10
        const isSourceSmaller = sourceNode.id < target.id;
        offset = isSourceSmaller ? -15 : 15;

        // 标签偏移：为了让标签不遮挡线，标签应该往外侧偏
        // 如果线往左偏(-15)，标签应该更往左偏(-10)
        labelOffset = isSourceSmaller ? -12 : 12;
      } else {
        // 单向：为了避免多条单向线重叠，也可以加一点随机偏移
        // 或者基于 Label 的 Hash
        const labelHash = label.length % 3;
        offset = (labelHash - 1) * 5; // -5, 0, 5
      }

      const { path, labelX, labelY, isVertical } = getManhattanPath(x1, y1, x2, y2, offset);
      ...
      // 绘制标签
      // 如果是双向关系，labelOffset 会让标签显示在线的两侧
      labelElements.push(drawLabel(labelX, labelY, label, isVertical, labelOffset));
    }
  });
  ...
  nodes.forEach(node => {
    ...
    g.addEventListener('click', e => {
      e.stopPropagation(); // 阻止冒泡
      dbg('[关系图节点] 被点击:', node.id);

      const char = allChars.find(c => c.name === node.id);
      if (char) {
        dbg('[关系图节点] 找到角色数据，准备弹出卡片:', char.name);
        showRelationCharDetail(char, e);
      } else {
        dbg('[关系图节点] 未找到角色数据:', node.id);
        // 尝试从 node.data 获取
        if (node.data) {
          dbg('[关系图节点] 使用 node.data 弹出:', node.data.name);
          showRelationCharDetail(node.data, e);
        }
      }
    });
...
// 显示角色详情气泡（改版：气泡菜单模式 - 类似兼容性表格Cell菜单）
...
function createSettingsUI() {
  try {
    ...
    const $header = $(
      `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><span style="font-weight:bold;font-size:16px;">浮岛设置 <span style="font-size:10px;font-weight:normal;opacity:0.6;">v1.1</span></span><span class="ci-close-btn">${ICONS.close}</span></div>`,
    );
