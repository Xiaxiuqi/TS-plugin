      ...
      <div class="ci-worldinfo-details-box">
        ${Object.entries(otherDetails)
          .map(([key, val]) => {
            ...
            if (key === '概览' || key === '摘要' || key === '大纲') return '';
            ...
            if (isDialogue) {
              ...
              // 过滤掉已经作为核心字段显示的列
              if (['概览', '摘要', '大纲'].includes(key)) return '';
              // 非对话列，维持原有的标签显示形式
...
function createSettingsUI() {
  try {
    ...
    const $header = $(
      `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-shrink:0;"><span style="font-weight:bold;font-size:16px;">浮岛设置 <span style="font-size:10px;font-weight:normal;opacity:0.6;">v1.4.5</span></span><span class="ci-close-btn">${ICONS.close}</span></div>`,
    );
